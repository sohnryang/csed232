...
void convert_pixel_type(const Image<RGBf> &src, Image<RGB8b> &dst) {
  ...
  for (int y = 0; y < src.height(); y++) {
    for (int x = 0; x < src.width(); x++) {
      for (int ch = 0; ch < 3; ch++) {
        dst[y][x][ch] = static_cast<uint8_t>(
            std::min(255.0f, std::max(0.0f, src[y][x][ch] * 255.0f)));
      }
    }
  }
}
...
struct BMPInfoHeader {
  ...
  uint16_t get_depth() const { return static_cast<uint16_t>(bit_count); }
};
#pragma pack(pop)
...
bool load_bmp(const char *path, Image<RGB8b> &img) {
  ...
  file.read(reinterpret_cast<char *>(&bmp_file_header),
  ...
  // We will use the width to determine how many bytes are needed to hold one
  // row of the image data We need to add padding bytes to ensure that each row
  // is aligned on a 4 byte boundary
  int padding =
      (4 - (bmp_info_header.width * bmp_info_header.get_depth() / 8) % 4) % 4;
...
bool save_bmp(const char *path, const Image<RGB8b> &img) {
  ...
  file.write(reinterpret_cast<const char *>(&bmp_file_header),
             sizeof(bmp_file_header));
  file.write(reinterpret_cast<const char *>(&bmp_info_header),
             sizeof(bmp_info_header));
